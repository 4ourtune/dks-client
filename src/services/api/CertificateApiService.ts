...
import { API_BASE_URL, API_TIMEOUTS, RETRY_CONFIG } from '@/utils/constants';
...
class CertificateApiServiceClass {
  ...
  private setupInterceptors() {
    ...
    this.api.interceptors.request.use(
      async (config) => {
        ...
        if (token && !config.url?.includes('root-ca')) {
    ...
    this.api.interceptors.response.use(
      ...
      async (error) => {
        if (this.shouldRetry(error) && this.retryCount < RETRY_CONFIG.MAX_ATTEMPTS) {
          this.retryCount++;
          const delay = RETRY_CONFIG.DELAY * Math.pow(RETRY_CONFIG.BACKOFF_FACTOR, this.retryCount - 1);
          
          console.log(`Retrying certificate API request (attempt ${this.retryCount}/${RETRY_CONFIG.MAX_ATTEMPTS})...`);
  ...
  async requestUserCertificate(request: CertificateRequest): Promise<UserCertificate> {
    try {
      ...
      if (!response.data?.certificate) {
        throw new Error('Invalid certificate response structure');
      }
      ...
      const certificate: UserCertificate = {
        ...
        serialNumber: response.data.certificate.serialNumber,
        version: response.data.certificate.version || 1,
        vehicleId: response.data.certificate.vehicleId,
        permissions: response.data.certificate.permissions,
        userId: response.data.certificate.userId
      };
  ...
  async getUserCertificates(): Promise<UserCertificate[]> {
    try {
      ...
      const certificates: UserCertificate[] = response.data.certificates.map((cert: any) => ({
        id: cert.id,
  ...
  async refreshCertificate(certificateId: string): Promise<UserCertificate> {
    try {
      ...
      const response = await this.api.post(`/certificates/${certificateId}/refresh`, {}, {
        timeout: API_TIMEOUTS.LONG
      });
