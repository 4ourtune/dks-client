...
export class BackgroundOptimizer {
  ...
  private readonly handleAppStateChangeBound = this.handleAppStateChange.bind(this);
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      this.appStateSubscription = AppState.addEventListener(
        "change",
        this.handleAppStateChangeBound,
      );
  ...
  private async scheduleRenewal(vehicleId: number, certificate: any): Promise<void> {
    try {
      ...
      await AsyncStorage.setItem(`renewal_${vehicleId}`, JSON.stringify(renewalInfo));
    } catch (error) {
